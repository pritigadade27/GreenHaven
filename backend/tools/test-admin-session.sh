...
check "wrong password refused" 400 "$(code -X POST $API/admin/auth/login -H 'Content-Type: application/json' -d '{"email":"admin@greenhaven.in","password":"nope-not-it"}')"
...
check "reason recorded as SUPERSEDED" 1 "$(Q "SELECT COUNT(*) FROM admin_session WHERE revoked_reason='SUPERSEDED';")"
...
check "INVENTORY_UPDATED logged" 1 "$(Q "SELECT COUNT(*) FROM admin_activity_log WHERE action='INVENTORY_UPDATED';")"
check "log records who" "admin@greenhaven.in" "$(Q "SELECT admin_email FROM admin_activity_log WHERE action='INVENTORY_UPDATED' ORDER BY id DESC LIMIT 1;")"
